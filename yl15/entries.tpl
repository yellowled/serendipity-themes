...
{foreach from=$entries item="dategroup"}
   ...
   {assign var="entry" value=$entry scope="parent"}
      ...
      {if $template_option.entryfooterposition == 'false'}
         <div class="serendipity_subtitlebelow">
         <a href="{$entry.link_author}">{$entry.author}</a>
         {if $entry.categories}
            {$CONST.IN} {foreach from=$entry.categories item="entry_category" name="categories"}<a href="{$entry_category.category_link}">{$entry_category.category_name|@escape}</a>{if not $smarty.foreach.categories.last}, {/if}{/foreach}{if $dategroup.is_sticky} ({$CONST.STICKY_POSTINGS}){/if} - {$entry.timestamp|@formatTime:'%a, %d.%m.%Y'} - {$entry.timestamp|@formatTime:'%H:%M'}
      ...
      {if $template_option.entryfooterposition == 'true' OR $head_version < 1.1}
         ...
         {if $entry.has_comments}
            {if $use_popups}
               <a href="{$entry.link_popup_comments}" onclick="window.open(this.href, 'comments', 'width=480,height=480,scrollbars=yes'); return false;">{if $entry.comments == 0}{$CONST.NO_COMMENTS}{else}{$entry.comments} {$entry.label_comments}{/if}</a><br /><a href="{$entry.link}#serendipity_CommentForm">{$CONST.ADD_COMMENT}</a>
            {else}
               <a href="{$entry.link}#comments">{if $entry.comments == 0}{$CONST.NO_COMMENTS}{else}{$entry.comments} {$entry.label_comments}{/if}</a>
               &ndash; <a href="{$entry.link}#serendipity_CommentForm">{$CONST.ADD_COMMENT}</a>
            {/if}
         {/if}
         {if $entry.has_trackbacks}
            {if $use_popups}
               &ndash; <a href="{$entry.link_popup_trackbacks}" onclick="window.open(this.href, 'comments', 'width=480,height=480,scrollbars=yes'); return false;">{if $entry.trackbacks == 0}{$CONST.NO_TRACKBACKS}{else}{$entry.trackbacks} {$entry.label_trackbacks}{/if}</a>
            {else}
               &ndash; <a href="{$entry.link}#trackbacks">{if $entry.trackbacks == 0}{$CONST.NO_TRACKBACKS}{else}{$entry.trackbacks} {$entry.label_trackbacks}{/if}</a>
            {/if}
         {/if}
      ...
      {/if}
      {if $template_option.entryfooterposition == 'false'}
         <div class="serendipity_entryFooter below_entry">
            {if $entry.has_comments}
               {if $use_popups}
                  <a href="{$entry.link_popup_comments}" onclick="window.open(this.href, 'comments', 'width=480,height=480,scrollbars=yes'); return false;">{if $entry.comments == 0}{$CONST.NO_COMMENTS}{else}{$entry.comments} {$entry.label_comments}{/if}</a>
               {else}
                  <a href="{$entry.link}#comments">{if $entry.comments == 0}{$CONST.NO_COMMENTS}{else}{$entry.comments} {$entry.label_comments}{/if}</a>
                  &ndash; <a href="{$entry.link}#serendipity_CommentForm">{$CONST.ADD_COMMENT}</a>
               {/if}
   ...
   <div class="serendipity_pageFooter" style="text-align: center">
      {if $footer_info}
         ...
         {else}
            <span class="not_active">&laquo; {$CONST.PREVIOUS_PAGE}</span>&#160;
         {/if}
      {else}
      {/if}
      ...
      {if $footer_info}
         ...
         {else}
            &#160;<span class="not_active">{$CONST.NEXT_PAGE} &raquo;</span>
         {/if}
      {else}
      {/if}

      <br />
      {if not $startpage}<a href="{$serendipityBaseURL}">{$CONST.ADMIN_FRONTPAGE}</a>{/if}
      {if not $footer_info} - <a href="#topofpage">{$CONST.TOP_LEVEL}</a>{/if}
