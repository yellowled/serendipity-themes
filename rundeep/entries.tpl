       ...
       <div class='serendipity_entryFooter'>
		{if $entry.categories}
			...
			{if $entry.categories}
                ...
                {/if}
			<br />
			{else}<a href="{$entry.link_author}">{$entry.author}</a>
                {if $entry.categories}
                   {$CONST.IN} {foreach from=$entry.categories item="entry_category" name="categories"}<a href="{$entry_category.category_link}">{$entry_category.category_name|@escape}</a>{if not $smarty.foreach.categories.last}, {/if}{/foreach}
                {/if}
                {if $dategroup.is_sticky}
		...
		{/if}
			{if $entry.is_entry_owner and not $is_preview}
                ...
                {if $entry.has_comments}
                    {if $use_popups}
                        <br /><a href="{$entry.link_popup_comments}" onclick="window.open(this.href, 'comments', 'width=480,height=480,scrollbars=yes'); return false;">{if $entry.comments == 0}{$CONST.NO_COMMENTS}{else}{$entry.comments} {$entry.label_comments}{/if}</a><br /><a href="{$entry.link}#serendipity_CommentForm">{$CONST.ADD_COMMENT}</a>
                    {else}
                        <br /><a href="{$entry.link}#comments">{if $entry.comments == 0}{$CONST.NO_COMMENTS}{else}{$entry.comments} {$entry.label_comments}{/if}</a>
                        <br /><a href="{$entry.link}#serendipity_CommentForm">{$CONST.ADD_COMMENT}</a>
                    {/if}
                {/if}

                {if $entry.has_trackbacks}
                    {if $use_popups}
                        <br /><a href="{$entry.link_popup_trackbacks}" onclick="window.open(this.href, 'comments', 'width=480,height=480,scrollbars=yes'); return false;">{if $entry.trackbacks == 0}{$CONST.NO_TRACKBACKS}{else}{$entry.trackbacks} {$entry.label_trackbacks}{/if}</a>
                    {else}
                        <br /><a href="{$entry.link}#trackbacks">{if $entry.trackbacks == 0}{$CONST.NO_TRACKBACKS}{else}{$entry.trackbacks} {$entry.label_trackbacks}{/if}</a>
                    {/if}
                {/if}
		{$entry.add_footer}
            </div>
